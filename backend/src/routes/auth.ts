...
router.post('/signup', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (schoolRes.rows.length === 0) return res.status(400).json({ error: 'Invalid config code' });
        ...
        await pgPool.query(
            'INSERT INTO users (school_id, name, email, password_hash, role) VALUES ($1, $2, $3, $4, $5)',
            [schoolId, name, email, hash, role.toUpperCase()]
        );
        ...
        try {
            await pgPool.query(
                ...
                [`${name} (${role.toUpperCase()}) has joined your school using the registration code.`, schoolId]
            );
        } catch (notifErr) {
    ...
    try {
        ...
        if (userRes.rows.length === 0) {
            return res.status(400).json({
                error:
                    'No account for that email. Register your school first (admin), or sign up with your school code.',
            });
        }
